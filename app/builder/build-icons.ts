...
const inputDir = path.join(cwd, "app", "assets", "icons");

const inputDirRelative = path.relative(cwd, inputDir);
const outputDir = path.join(cwd, "app", "components", "sugar", "icons");
const outputSvg = path.join(cwd, "public");
...
async function generateIconFiles() {
  const spriteFilepath = path.join(outputSvg, "sprite.svg");
...
`;
  ...
  // Manifest confused to extension/PWA
  logVerbose(`Types generated icons saved to ${path.relative(cwd, typeOutputFilepath)}`);
...
async function writeIfChanged(filepath: string, newContent: string) {
  ...
  // NOTE : fix invoke prettier on CI
  await $`npx prettier --write ${filepath} --ignore-unknown`;
  return true;
}
